...
This revision incorporates the first implementation pressure-test of the design system:
- strong-model generation now begins only after auth; pre-auth prompt/inspiration state must survive OAuth;
- setup progress is aligned with actual publish-readiness rules rather than a fixed 4-item completion count;
- tertiary app text contrast is corrected;
- renderer sections expose a stable collaborator-action anchor;
- Preview Mode gets a Mobile/Desktop width toggle on larger screens;
- mobile concept previews lazy-mount for performance;
- the behavioral prototype has a canonical repository path;
- Inter variable/static-weight behavior is explicit;
- application chrome is light-only for MVP;
- the event footer is explicitly part of renderer output;
- co-host invitation acceptance is specified.
...
Every published guest site includes the tasteful product referral footer required by the PRD.

The footer:
- consumes event tokens so it feels visually integrated;
- remains intentionally low emphasis;
- contains the approved `Made with …` product attribution/link;
- must not compete with the event content;
- remains visible in guest preview;
- is not an editable host content block in MVP.

## 15.15 No decorative site imagery in MVP
