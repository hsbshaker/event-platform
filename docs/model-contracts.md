...
| `eval:design-intent-challenge` | `designIntentChallenge` | `design-intent-sealed-challenge-v4.json` | `docs/model-evals/results/design-intent-sealed-challenge-v4` | **sealed challenge, the replacement** — its predecessor was authored, passed every mechanical control, and was then invalidated before its freeze and before any provider call, because the system-aware fairness review found the reviewer *context* inadequate rather than the cases wrong. A corpus a system-aware reviewer has read is no longer sealed, whatever the packet is later corrected to say. This one is authored after that correction freezes, by **two external model families** — six cases each, none from the model family that authored every prior corpus in this programme — under a protocol frozen before either wrote a case. The generalization evidence the §3.7 gate is applied to. One run, then spent |
...
own side. T22 writes the sealed challenge, after the implementation freeze, and **its authoring protocol is itself frozen first**. Three corpora were invalidated before one ran. The last of them, v3, was commissioned from two external model families and failed as a *method* rather than a corpus: a fresh isolated session, shown no prior case and deliberately given no list of subjects to avoid, reproduced a discarded half of its own almost slot for slot. Its surviving half passed both reviews and is **still** not carried forward, because keeping it while swapping only the failed family would make the corpus adaptive to observed review outcomes. **v4 therefore freezes the human situation before the design vocabulary is shown at all**: six *situation cards* per half naming who gathers, why it matters and what complicates it — and no colour, typeface, motif, style or layout — frozen before either author sees the EventIdentity contract, then mapped into cases that must stay faithful to them. Half A is one fresh human author and half B one fresh model family from neither of the families already represented here, chosen and recorded before commissioning; Claude authors nothing. The namespaces `DIC4-P` and `DIC4-Q` are neutral so no reviewer learns which half a human wrote, the values live in `src/lib/ai/evals/design-intent-challenge-v4-protocol.ts`, and v3 is preserved entire as evidence — about benchmark authoring distributions, not about DesignIntent, which never ran. Adding a corpus file is then the entire change: no runner, checker,
