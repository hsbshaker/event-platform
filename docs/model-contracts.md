...
| `eval:design-intent-challenge` | `designIntentChallenge` | `design-intent-sealed-challenge-v3.json` | `docs/model-evals/results/design-intent-sealed-challenge-v3` | **sealed challenge, the replacement** — its predecessor was authored, passed every mechanical control, and was then invalidated before its freeze and before any provider call, because the system-aware fairness review found the reviewer *context* inadequate rather than the cases wrong. A corpus a system-aware reviewer has read is no longer sealed, whatever the packet is later corrected to say. This one is authored after that correction freezes, by an author who has seen neither the implementation nor the invalidated candidate. The generalization evidence the §3.7 gate is applied to. One run, then spent |
