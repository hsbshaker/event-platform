...
own side. T22 writes the sealed challenge, after the implementation freeze, and **its authoring protocol is itself frozen first**: the second candidate was invalidated not for contamination but for *distributional* dependence — a fresh session buys a fresh context, not a fresh authoring distribution — so the v3 corpus is written externally by two model families that have authored none of this programme's corpora, six cases each and none by the lead, with the namespace, the split, the per-half composition rule, the assembly order and the corpus version fixed in `src/lib/ai/evals/design-intent-challenge-protocol.ts` and `docs/phase-4b-plan.md` Part IV while no case existed. The family labels are user-supplied provenance rather than a cryptographic claim, and they never reach the model or the blind reviewer. Adding a corpus file is then the entire change: no runner, checker,
