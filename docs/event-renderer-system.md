...
Runs in a headless browser against the production renderer (see `docs/technology-decisions.md`). Each text node's line count and bounds are measured at both breakpoints; nodes over their limit (display and primary: three lines at desktop, four at mobile) or overflowing their container are demoted one emphasis step and the page is re-rendered, up to three rounds; if any overflow remains, the innermost `Frame`/`Surface`/`Rail` around the node is relaxed (Frame → Stack, Surface inset → tight, Rail widened), logged as `fit.verified.structural`. The renderer's stylesheet also carries a floor (`overflow-wrap: anywhere`, stepped down to `break-word` on headings, wrapping glyph rows, clipped decorations, rail-sized numerals) so a page cannot overflow horizontally even before verification. Both break a word that cannot fit a line of its own, and no container is sized by its content, so the floor is the same either way; `break-word` only stops a word being broken while whole words would still have fitted, which at display sizes reads as a fault. A spec is final only with `verified.clean === true`.

**Containment is not composition.** The floor above succeeds by trading an overflow verification would catch for a wrap it did not measure, so a page could be geometry-clean and still read as a rendering fault (`docs/human-test-1/qualitative-findings.md`, F1). Verification therefore also measures each text node's **rendered line boxes** — from `Range.getClientRects()`, grouped by vertical position — and records three further defects, all of which must be zero for a spec to be final:

| Defect | Rule | Repair |
| --- | --- | --- |
| `textWordBroken` | Line boxes exceed the node's count of **unbreakable segments** — runs of text with no break opportunity inside them, so whitespace, hyphens, dashes and slashes all end one. Laying `s` segments on `n` lines takes `n - 1` breaks and the text offers only `s - 1`, so `n > s` proves at least one break landed inside a segment. This is the minimum usable measure, stated without a threshold. | Demote, then relax the innermost box. |
| `textOverMetadataLimit` | An atomic metadata value (`Date`, `Time`, `Venue`, `Location`) at `secondary` or `caption` past `max(2 desktop / 3 mobile, ⌈words ÷ 3⌉)` line boxes. `FIT_LIMITS` governs `display` and `primary`; this covers the emphases where §3.1 was silent. The word term makes it a fragmentation limit rather than a length limit. | Relax the innermost box. Demotion is not a repair here: the budget applies only at or below the demotion floor. |
| `textEdgeIncoherent` | Line boxes that do not share their aligned edge — left for start-aligned text, centre for centred, right for end-aligned — beyond the sub-pixel tolerance. `EventTitle` under `stagger` or `cascade` is exempt: those are the only treatments that displace a line, and the stylesheet bounds their offsets. | None. A displaced line is not a narrow one and no box change moves it back, so the concept is reported unclean rather than re-rendered. |

`EventTitle.layout` is `block`, `stagger` or `cascade`. The model chooses the treatment and nothing else: **where the title breaks is the compiler's** (`@/lib/renderer/compile/title-lines`), chosen deterministically by scoring every way to cut the words into lines and preferring even lines that stay near the measure a display line wants, with no line of pure function words, no break after a preposition or article, and no orphaned last word. A title short enough for one line keeps it. `stagger` then offsets the odd lines and `cascade` ramps from the second — different lines, so the two stay distinct on a two-line title, which is what most titles break into — never by flipping a line's alignment, which is the artifact F1 records. The offsets are **lengths in `em`, never percentages**: CSS resolves percentage padding to zero during intrinsic sizing, so a title leaf that shrink-wraps to its own content would be measured as though the indent were not there and would then lose exactly the indent from its longest line, breaking a word that fits. The model never sees a break point, an offset or a pixel.

Line boxes are measured from a range over each **text node**, not one range over the element. `Range.getClientRects()` on an element's contents also returns the border box of every element inside it, and for a treated title those boxes span the full measure and start at its edge — so an indented line would read as flush and a fragmented one as wide.
