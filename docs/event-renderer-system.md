...
| Defect | Rule | Repair |
| --- | --- | --- |
| `textWordBroken` | Line boxes exceed the node's word count. Laying `w` words on `n` lines takes `n - 1` breaks and only `w - 1` fall between words, so `n > w` proves at least one break landed inside a word. This is the minimum usable measure, stated without a threshold. | Demote, then relax the innermost box. |
| `textOverMetadataLimit` | An atomic metadata value (`Date`, `Time`, `Venue`, `Location`) at `secondary` or `caption` past `max(2 desktop / 3 mobile, ⌈words ÷ 3⌉)` line boxes. `FIT_LIMITS` governs `display` and `primary`; this covers the emphases where §3.1 was silent. The word term makes it a fragmentation limit rather than a length limit. | Demote where possible, then relax the innermost box. |
...
`EventTitle.layout` is `block`, `stagger` or `cascade`. The model chooses the treatment and nothing else: **where the title breaks is the compiler's** (`@/lib/renderer/compile/title-lines`), chosen deterministically by scoring every way to cut the words into lines and preferring even lines, no line of pure function words, no break after a preposition or article, and no orphaned last word. A title short enough for one line keeps it. `stagger` then offsets alternate lines and `cascade` ramps them, both by a bounded percentage of the title's own measure — never by flipping a line's alignment, which is the artifact F1 records. The model never sees a break point, an offset or a pixel.
