...
export function buildMechanicalReport(
  ...
  const lines: string[] = [
    ...
    `| Latency p95 | ${percentile(latencies, 95)} ms (nearest-rank; on a sample this small this is the maximum) |`,
    `| Latency min / max | ${latencies.length === 0 ? "—" : `${Math.min(...latencies)} / ${Math.max(...latencies)} ms`} |`,
...
export function buildBlindArtifact(runs: CaseRun[]): string {
  const lines: string[] = [
    ...
    "",
    `Below are ${runs.length} host descriptions and the brief the system produced for each.`,
    "",
