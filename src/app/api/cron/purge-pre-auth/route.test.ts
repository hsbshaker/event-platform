...
// At least 32 characters, because `cronSecret()` rejects anything shorter: it is the only
// credential in front of a service-role endpoint.
...
describe("purge-pre-auth", () => {
  ...
  it("stays closed, and stays quiet, when the secret itself is misconfigured", async () => {
    // A secret too short to be one must not become a 500 that confirms this route exists to
    // whoever is probing. It fails closed like an unset secret and shouts in the logs instead.
    const logged = vi.spyOn(console, "error").mockImplementation(() => {});
    try {
      vi.stubEnv("CRON_SECRET", "x");
      expect((await run(`Bearer x`)).status).toBe(404);
      expect((await run(null)).status).toBe(404);
      expect(rpc).not.toHaveBeenCalled();
  ...
  it("is closed when no secret is configured at all", async () => {
    vi.stubEnv("CRON_SECRET", "");
    expect((await run(null)).status).toBe(404);
    expect((await run(`Bearer ${SECRET}`)).status).toBe(404);
    expect(rpc).not.toHaveBeenCalled();
  });
