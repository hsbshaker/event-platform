...
Validation is deterministic and set-level: the premises must be distinct in name and in organizing idea, no two may sit at the same register on all three axes, and any axis the set declares constrained must really be uniform. The register rule is an anti-collapse floor and nothing more — semantic distinctness is the organizing idea's, and two premises sharing one or two register axes is ordinary. A stricter earlier form, requiring an axis to separate all three, was corrected: it refused 264 honest register configurations for every collapsed one it caught, and its only escape was to move a register the idea had not asked to move. The set may be re-prompted **once**, with the validator's own findings quoted; a set that is still unusable fails the batch visibly rather than reverting to three premise-free calls, because that reversion is the known-defective behaviour the evidence run measured.
...
- [ ] The premise set is refused unless the three are distinct in name and in organizing idea and no two sit at the same register on all three axes; a declared axis constraint is checked against the set.
