...
| `npm run test:db`    | Migration + RLS integration suite against `TEST_DATABASE_URL` (Postgres 16+); installs an `auth` schema stub, applies `supabase/migrations`, and exercises policies as `anon`, `authenticated` and `service_role` |
