...
const ENUM = {
    Ratio: ["38", "50", "62"], RailWidth: ["thin", "medium", "wide"], BandHeight: ["thin", "medium", "tall"],
    Inset: ["tight", "normal", "deep"], Gap: ["tight", "normal", "loose"], Align: ["start", "center", "end"],
    Justify: ["start", "center", "end", "between"], Emphasis: ["display", "primary", "secondary", "caption"],
    SurfaceRole: ["base", "alt", "contrast", "accent"], RuleWeight: ["hairline", "strong", "double"],
    Anchor: ["top-start", "top-end", "bottom-start", "bottom-end", "center"], Extent: ["quarter", "third", "half", "full"],
    MotifId: ["plaid", "stripe", "gingham", "linen", "equestrian", "botanical", "celestial"], MotifRole: ["field", "band", "frame", "divider", "accent"],
};
...
const textProps = (doc) => ({ kind: "text", doc, props: { emphasis: { enum: ENUM.Emphasis }, case: { enum: ["upper", "none"] } } });
const NODE_SPEC = {
...
function childrenOf(n) {
    ...
    if (Array.isArray(a.children))
        a.children.forEach((c, i) => out.push({ key: `children[${i}]`, node: c }));
    for (const k of ["child", "rail", "content", "decoration", "layout"])
        if (a[k] && typeof a[k] === "object")
            out.push({ key: k, node: a[k] });
    return out;
}
function walk(tree, fn) {
    tree.sections.forEach((s, si) => {
        const rec = (node, path, parent, parentKey, ancestors) => {
            fn({ node, path, parent, parentKey, ancestors, sectionIndex: si });
            for (const c of childrenOf(node))
                rec(c.node, `${path}.${c.key}`, node, c.key, [...ancestors, node]);
        };
        rec(s.root, `sections[${si}].root`, null, "section", []);
    });
}
function count(tree, pred) { let n = 0; walk(tree, v => { if (pred(v))
    n++; }); return n; }
function isText(n) { return TEXT_KINDS.includes(n.t) || n.t === "Date" || n.t === "SectionHeading"; }
function hasTextDescendant(n) { if (isText(n))
    return true; return childrenOf(n).some(c => hasTextDescendant(c.node)); }
function containerDepth(ancestors) { return ancestors.filter(a => CONTAINERS.includes(a.t)).length; }
function clone(x) { return JSON.parse(JSON.stringify(x)); }
function fnv(s) { let h = 2166136261; for (let i = 0; i < s.length; i++) {
    h ^= s.charCodeAt(i);
...
function validateSchema(json) {
    const errors = [];
    const err = (rule, path, detail) => errors.push({ rule, path, detail });
    const checkNode = (n, path, depth) => {
        if (depth > 12) {
        ...
        const spec = NODE_SPEC[n.t];
        if (!spec)
            return err("schema.type", path, `unknown node type ${JSON.stringify(n.t)}`);
        for (const k of Object.keys(n))
            if (k !== "t" && !spec.props[k] && !(k === "children" && spec.children))
                err("schema.key", `${path}.${k}`, `unknown key on ${n.t}`);
        for (const [k, p] of Object.entries(spec.props)) {
            const v = n[k];
            if (v === undefined) {
                if (p.required)
                    err("schema.required", `${path}.${k}`, `${n.t} requires ${k}`);
                continue;
            }
            if (p.enum) {
                if (!p.enum.includes(v))
                    err("schema.enum", `${path}.${k}`, `${JSON.stringify(v)} not in ${p.enum.map(x => JSON.stringify(x)).join("|")}${typeof v === "number" && p.enum.some(x => typeof x === "string") ? " (must be a string)" : ""}`);
            }
            else if (p.type === "motif") {
                if (!v || typeof v !== "object" || !ENUM.MotifId.includes(v.id) || !ENUM.MotifRole.includes(v.role) || Object.keys(v).some(x => x !== "id" && x !== "role"))
                    err("schema.motif", `${path}.${k}`, "motif must be {id, role} from the motif list");
            }
            else if (p.type === "node")
                checkNode(v, `${path}.${k}`, depth + 1);
        }
        if (spec.children) {
            if (!Array.isArray(n.children))
                err("schema.children", `${path}.children`, `${n.t} requires children[]`);
            else {
                if (n.children.length < spec.children.min || n.children.length > spec.children.max)
                    err("schema.children.count", `${path}.children`, `${n.t} takes ${spec.children.min}..${spec.children.max} children, got ${n.children.length}`);
                n.children.forEach((c, i) => checkNode(c, `${path}.children[${i}]`, depth + 1));
            }
        }
        else if (n.children !== undefined)
            err("schema.key", `${path}.children`, `${n.t} has no children`);
    };
    const t = json;
    if (!t || typeof t !== "object")
        return { ok: false, errors: [{ rule: "schema.root", path: "", detail: "not an object" }] };
    if (t.version !== "composition_v1")
        err("schema.version", "version", "must be composition_v1");
    for (const k of Object.keys(t))
        if (!["version", "sections"].includes(k))
            err("schema.key", k, "unknown top-level key");
    if (!Array.isArray(t.sections))
        err("schema.sections", "sections", "missing sections[]");
    else
        t.sections.forEach((s, i) => {
            const p = `sections[${i}]`;
            if (!s || typeof s !== "object")
                return err("schema.section", p, "not an object");
            for (const k of Object.keys(s))
                if (!["kind", "surface", "align", "fill", "root"].includes(k))
                    err("schema.key", `${p}.${k}`, "unknown section key");
            if (!SECTION_SPEC.kind.includes(s.kind))
                err("schema.enum", `${p}.kind`, `kind must be ${SECTION_SPEC.kind.join("|")}`);
            if (!SECTION_SPEC.surface.includes(s.surface))
                err("schema.enum", `${p}.surface`, "surface must be base|alt|contrast|accent");
            if (s.align !== undefined && !SECTION_SPEC.align.includes(s.align))
                err("schema.enum", `${p}.align`, "align must be start|center|end");
            if (s.fill !== undefined && !SECTION_SPEC.fill.includes(s.fill))
                err("schema.enum", `${p}.fill`, "fill must be auto|screen");
            if (s.root === undefined)
                err("schema.required", `${p}.root`, "section requires root");
            else
                checkNode(s.root, `${p}.root`, 0);
        });
    return { ok: errors.length === 0, errors };
}
// ============================================================ 2. structural validation (returns violations; repair() fixes them)
function validateStructure(tree, caps) {
    const v = [];
    const add = (rule, path, detail) => v.push({ rule, path, detail });
    const secs = tree.sections;
    if (secs.length < LIMITS.sectionsMin || secs.length > LIMITS.sectionsMax)
        add("sections.count", "sections", `${secs.length} sections; allowed ${LIMITS.sectionsMin}..${LIMITS.sectionsMax}`);
    ...
    for (const k of ["hero", "details", "rsvp", "registry", "band"]) {
        ...
        if (n > 1)
            add("sections.duplicateKind", "sections", `${n} sections of kind ${k}`);
    }
    ...
    if (caps.registry && !kinds.includes("registry"))
        add("coverage.registrySection", "sections", "registry is enabled but there is no registry section");
    if (!caps.registry && kinds.includes("registry"))
        add("capability.registrySection", "sections", "registry is not enabled");
    let run = 0;
    secs.forEach((s, i) => { run = s.surface === "contrast" ? run + 1 : 0; if (run > 2)
    ...
    walk(tree, ({ node, path, parent, parentKey, ancestors, sectionIndex }) => {
        ...
        (seen[_a = node.t] || (seen[_a] = [])).push({ path, node });
        const d = containerDepth(ancestors);
        if (d > LIMITS.depth)
            add("depth", path, `container depth ${d} > ${LIMITS.depth}`);
        ...
        if (node.t === "Rule" && pt === "Cluster" && node.orientation !== "v")
        ...
        if (parentKey === "rail") {
            ...
            else if (node.t === "Stack" && (node.children.length > 4 || node.children.some(c => !INLINE_LEAVES.includes(c.t))))
        ...
        if (node.t === "Grid" && !inRegistry && ancestors.some(a => a.t === "Grid" && !ancestors.some(b => b.t === "Registry")))
            add("nesting.gridInGrid", path);
        if (node.t === "Rail" && ancestors.some(a => a.t === "Rail"))
            add("nesting.railInRail", path);
        if (node.t === "Split" && ancestors.filter(a => a.t === "Split").length >= LIMITS.splitNesting)
            add("nesting.splitDepth", path, "Split nested more than once");
        if (node.t === "Surface") {
            const eff = [...ancestors].reverse().find(a => a.t === "Surface");
            const role = eff ? eff.role : secs[sectionIndex].surface;
            if (node.role === role)
                add("nesting.surfaceSame", path, `Surface ${role} inside ${role}`);
        }
        if (parentKey === "content" && !(["Stack", "Frame", "Split"].includes(node.t) && hasTextDescendant(node)))
            add("nesting.overlayContent", path, "Overlay content must be a text-bearing Stack, Frame or Split");
        if (parentKey === "decoration") {
            const ok = node.t === "MotifField" || node.t === "Monogram" || node.t === "Glyph" || (node.t === "Date" && node.form === "numeral");
            if (!ok)
                add("nesting.overlayDecoration", path, "decoration must be MotifField, Monogram, Glyph or Date numeral");
        }
        ...
        if (inRegistry && parentKey !== "layout" && !["Grid", "Cell", "Stack", "Split", "RegistryItem"].includes(node.t))
        ...
        if (COMPONENTS.includes(node.t)) {
            if (!COMPONENT_PARENTS.includes(pt))
                add("component.parent", path, `${node.t} inside ${pt}`);
            if (pt === "Cell") {
            ...
            if (pt === "Split") {
                const sp = parent;
                const idx = sp.children.indexOf(node);
                const share = idx === 0 ? Number(sp.ratio) : 100 - Number(sp.ratio);
                if (share < 50)
                    add("component.splitShare", path, `component gets ${share}% of a Split`);
            }
        ...
        if (node.t === "Split" && node.mobile === "keep") {
            const heavy = node.children.some(c => COMPONENTS.includes(c.t) || findFirst(c, "Description") || findFirst(c, "RSVP") || findFirst(c, "Registry"));
            if (heavy)
                add("responsive.splitKeep", path, "keep demoted: a side holds a component or the description");
        }
        ...
        if (node.t === "Time" && !caps.time)
            add("capability.node", path, "Time not available");
        ...
        if (node.t === "RSVP" && !caps.rsvp)
            add("capability.node", path, "RSVP not available");
        if (node.t === "Registry" && !caps.registry)
            add("capability.node", path, "Registry not available");
        if (node.t === "CTA" && node.target === "registry" && !caps.registry)
            add("capability.node", path, "CTA registry not available");
        if (node.t === "CTA" && node.target === "rsvp" && !caps.rsvp)
            add("capability.node", path, "CTA rsvp not available");
        if (node.t === "SectionHeading" && node.for === "rsvp" && !caps.rsvp)
            add("capability.node", path, "rsvp heading not available");
        if (node.t === "SectionHeading" && node.for === "registry" && !caps.registry)
            add("capability.node", path, "registry heading not available");
        if (node.t === "RegistryItem") {
            const k = node.kind;
            if ((k === "gift" && !caps.gifts) || (k === "external" && !caps.externalRegistry) || (k === "cashfund" && !caps.cashFund))
                add("capability.node", path, `registry item ${k} not available`);
        }
    });
    nodesPerSec.forEach((n, i) => { if (n > LIMITS.nodesPerSection)
    ...
    perSec.forEach((c, i) => { for (const [k, max] of Object.entries(LIMITS.perSection))
        if ((c[k] || 0) > max)
            add("limits.perSection", `sections[${i}]`, `${c[k]} ${k} > ${max}`); });
    for (const [k, max] of Object.entries(LIMITS.perPage))
        if ((perPage[k] || 0) > max)
            add("limits.perPage", "sections", `${perPage[k]} ${k} > ${max}`);
    // coverage
    const n = (t) => (seen[t] || []).length;
    if (n("EventTitle") !== 1)
        add(n("EventTitle") ? "coverage.duplicate" : "coverage.missing", "sections", `EventTitle appears ${n("EventTitle")} times`);
    if (n("Venue") < 1)
        add("coverage.missing", "sections", "Venue missing");
    if (n("Date") < 1)
        add("coverage.missing", "sections", "Date missing");
    for (const t of ["Monogram", "CashFund", "RSVP", "Registry", "Description"])
        if (n(t) > 1)
            add("coverage.duplicate", "sections", `${t} appears ${n(t)} times`);
    // per section: each text node at most once; Date at most twice with distinct forms; CTA at most once; one heading
    secs.forEach((_s, i) => {
        const inSec = (t) => (seen[t] || []).filter(x => x.path.startsWith(`sections[${i}]`));
        for (const t of ["Eyebrow", "Hosts", "Time", "Location", "Deadline", "Venue", "SectionHeading", "CTA"])
            if (inSec(t).length > 1)
                add("coverage.duplicate", `sections[${i}]`, `${t} appears ${inSec(t).length} times in one section`);
        const dates = inSec("Date");
        if (dates.length > 2 || new Set(dates.map(x => x.node.form)).size < dates.length)
            add("coverage.duplicate", `sections[${i}]`, "Date at most twice per section, with distinct forms");
    });
    if (caps.rsvp && n("RSVP") < 1)
        add("coverage.missing", "sections", "RSVP missing");
    if (caps.registry && n("Registry") < 1)
        add("coverage.missing", "sections", "Registry missing");
    const items = (seen["RegistryItem"] || []).map(x => x.node.kind);
...
function countText(n) { let c = isText(n) ? 1 : 0; for (const ch of childrenOf(n))
    c += countText(ch.node); return c; }
const DEFAULT_MACROS = {
    ...
    registrySection: (_s, caps) => ({ kind: "registry", surface: "alt", root: { t: "Stack", children: [{ t: "SectionHeading", for: "registry" }, { t: "Registry", layout: { t: "Grid", columns: 3, mobile: 1, children: ["gift", "external", "cashfund"].filter(k => (k === "gift" && caps.gifts) || (k === "external" && caps.externalRegistry) || (k === "cashfund" && caps.cashFund)).map(k => ({ t: "Cell", child: { t: "RegistryItem", kind: k } })) } }] } }),
};
function repair(input, caps, seed = 1, macros = DEFAULT_MACROS) {
    const tree = clone(input);
    const repairs = [];
    const log = (rule, path, kind, before, after) => repairs.push({ rule, path, kind, before, after });
    ...
    const getAt = (path) => { const parts = path.match(/[^.\[\]]+/g); let o = tree; for (const p of parts)
        o = o[p]; return o; };
    const removeAt = (path) => { const parts = path.match(/[^.\[\]]+/g); let o = tree; for (let i = 0; i < parts.length - 1; i++)
        o = o[parts[i]]; const last = parts[parts.length - 1]; if (Array.isArray(o))
        o.splice(Number(last), 1);
    else
        delete o[last]; };
    const leavesOf = (n) => childrenOf(n).length ? childrenOf(n).flatMap(c => leavesOf(c.node)) : [n];
    const contentOf = (n) => n.t === "Overlay" ? n.content : n.t === "Cell" ? n.child : n.child || n.children?.[0] || n;
    const pass = (label) => {
                ...
                try {
                    if (r === "sections.heroFirst") {
                        ...
                        if (tree.sections.length > LIMITS.sectionsMax) {
                            ...
                            tree.sections.push({ kind: "details", surface: "base", root: { t: "Stack", children: [{ t: "SectionHeading", for: "details" }, { t: "Cluster", children: [{ t: "Date", form: "full" }, { t: "Venue" }] }] } });
                        ...
                        for (let k = parts.length - 1; k >= 2 && !done; k--) {
                            const np = parts.slice(0, k).join("."), pp = parts.slice(0, k - 1).join(".");
                            const n = getAt(np), par = getAt(pp);
                        ...
                        const n = getAt(p);
                        const parts = p.split(".");
                        const cl = getAt(parts.slice(0, -1).join("."));
                        ...
                        if (cl.children.length < 2) {
                            setAt(parts.slice(0, -1).join("."), { t: "Stack", children: cl.children.length ? cl.children : [{ t: "Venue" }] });
                        }
                        ...
                        const n = getAt(p);
                        const leaves = leavesOf(n).filter(l => INLINE_LEAVES.includes(l.t)).slice(0, 4);
                        setAt(p, leaves.length ? { t: "Stack", children: leaves } : { t: "MotifField", motif: { id: "linen", role: "field" } });
                    ...
                    else if (r === "nesting.cellChild" || r === "nesting.gridInGrid" || r === "nesting.overlayInOverlay" || r === "nesting.railInRail" || r === "nesting.splitDepth" || r === "nesting.frameInFrame") {
                        const n = getAt(p);
                        const rep = n.t === "Overlay" ? n.content : n.t === "Grid" ? { t: "Stack", children: n.children.map((c) => c.child) } : n.t === "Rail" ? n.child : n.t === "Frame" ? { t: "Stack", children: [n.child] } : n.t === "Split" ? { t: "Stack", children: n.children } : n.child || n;
                        setAt(p, rep);
                        ...
                        const n = getAt(p);
                        setAt(p, hasTextDescendant(n) ? { t: "Stack", children: [n] } : { t: "Stack", children: [n, { t: "EventTitle", emphasis: "display" }] });
                        ...
                        setAt(p, { t: "Stack", children: items.length ? items : [{ t: "RegistryItem", kind: "gift" }] });
                        ...
                        const parts = p.split(".");
                        const sp = getAt(parts.slice(0, -1).join("."));
                        sp.ratio = "50";
                        ...
                        if (!s) {
                            s = { kind: k, surface: "base", root: { t: "Stack", children: [] } };
                        ...
                        const order = ["Glyph", "Rule", "MotifBand", "MotifField", "Monogram", "Deadline", "Location", "Eyebrow", "Time", "Description", "Hosts"];
                        let dropped = false;
                        ...
                        const rep = n.t === "Overlay" ? n.content : n.t === "Frame" ? { t: "Stack", children: [n.child] } : n.t === "Rail" ? n.child : n.t === "Grid" ? { t: "Stack", children: n.children.map((c) => c.child) } : { t: "Rule", weight: "hairline" };
                        setAt(target, rep);
                        ...
                        if (t === "RegistryItem") {
                            ...
                            const forms = new Set();
                            filter = n => { const f = n.form; if (forms.has(f))
                                return true; forms.add(f); return false; };
                        }
                        const scope = p.startsWith("sections[") ? p : "";
                        const paths = [];
                        walk(tree, v => { if (v.node.t === t && v.path.startsWith(scope) && filter(v.node))
                            paths.push(v.path); });
                        const victim = t === "Date" && paths.length ? paths[paths.length - 1] : paths[1] || paths[0];
                        if (!victim)
                            continue;
                        ...
                        if (Array.isArray(parent) && parent.length > 1)
                            ...
                            setAt(victim, { t: "Rule", weight: "hairline" });
                        ...
                        if (what === "EventTitle") {
                            ...
                            (s.root.t === "Stack" ? s.root.children : (s.root = { t: "Stack", children: [s.root] }).children).push({ t: "RSVP" });
                            log(r, p, "coverage", undefined, "RSVP appended");
                        }
                        else if (what === "Registry") {
                            const s = tree.sections.find(x => x.kind === "registry");
                            const items = ["gift", "external", "cashfund"].filter(k => (k === "gift" && caps.gifts) || (k === "external" && caps.externalRegistry) || (k === "cashfund" && caps.cashFund && !count(tree, v => v.node.t === "CashFund")));
                            (s.root.t === "Stack" ? s.root.children : (s.root = { t: "Stack", children: [s.root] }).children).push({ t: "Registry", layout: { t: "Stack", children: items.map(k => ({ t: "RegistryItem", kind: k })) } });
                            log(r, p, "coverage", undefined, "Registry appended");
                        }
                        else {
                            const node = what === "Venue" ? { t: "Venue" } : { t: "Date", form: "full" };
                            ...
                            if (cl)
                                cl.children.push(node);
                            else if (root.t === "Stack")
                                root.children.push({ t: "Cluster", children: [node, { t: "Time" }] });
                            else
                                s.root = { t: "Stack", children: [s.root, { t: "Cluster", children: [node, { t: "Time" }] }] };
...
function countNodes(n) { return 1 + childrenOf(n).reduce((a, c) => a + countNodes(c.node), 0); }
function findLast(tree, si, t) { let p = null; walk(tree, v => { if ((si < 0 || v.sectionIndex === si) && v.node.t === t)
    p = v.path; }); return p; }
function findFirst(n, t) { if (n.t === t)
    return n; for (const c of childrenOf(n)) {
...
function textFor(node, content) {
    if (node.t === "Date") {
        const d = node;
        return d.form === "numeral" ? content.dayNumeral : d.form === "month-year" ? `${content.monthShort} ${content.year}` : d.form === "weekday" ? content.weekday : content.date;
    }
    if (node.t === "SectionHeading")
        return { details: "Join us at the lodge.", rsvp: "Will you be there?", registry: "A few things we love." }[node.for];
    if (node.t === "CTA")
        return node.target === "rsvp" ? "RSVP" : "Registry";
    return content[node.t.toLowerCase()] || "";
}
function widthShares(tree, mode) {
    // fraction of the section width available to each node, by path
    const shares = {};
    const rec = (n, path, share) => {
        shares[path] = share;
        const a = n;
        if (n.t === "Split") {
        ...
        if (n.t === "Rail") {
            const rw = { thin: .08, medium: .15, wide: .25 }[a.width];
        ...
        if (n.t === "Grid") {
            ...
            a.children.forEach((c, i) => rec(c, `${path}.children[${i}]`, share * Math.min(1, (c.span || 1) / cols)));
            return;
        }
        if (n.t === "Frame") {
            const ins = { tight: .04, normal: .08, deep: .14 }[a.inset];
            rec(a.child, `${path}.child`, share * (1 - 2 * ins));
            return;
        }
        if (n.t === "Overlay") {
            rec(a.content, `${path}.content`, share);
            rec(a.decoration, `${path}.decoration`, share * { quarter: .25, third: .33, half: .5, full: 1 }[a.extent]);
            return;
        }
        for (const c of childrenOf(n))
            rec(c.node, `${path}.${c.key}`, share);
    };
    tree.sections.forEach((s, i) => rec(s.root, `sections[${i}].root`, 1));
    return shares;
}
function estimateFit(tree, content, metrics) {
    ...
    for (const mode of ["desktop", "mobile"]) {
        ...
        walk(out, ({ node, path }) => {
            if (!isText(node))
                return;
            const a = node;
            let emph = a.emphasis || (node.t === "EventTitle" ? "display" : node.t === "SectionHeading" ? "primary" : "secondary");
            const text = textFor(node, content);
            if (!text)
                return;
            for (let i = 0; i < 3; i++) {
                const limit = FIT_LIMITS[mode][emph];
                if (!limit)
                    break;
                ...
                if (lines <= limit)
                    break;
                const next = emph === "display" ? "primary" : emph === "primary" ? "secondary" : "caption";
                repairs.push({ rule: "fit.estimate", path, kind: "fit-estimate", before: `${emph} (${mode}: ~${lines} lines)`, after: next });
                a.emphasis = next;
                emph = next;
            }
        });
...
function canonicalize(tree) {
    ...
    walk(out, ({ node, path, sectionIndex }) => {
        ...
        for (const [k, v] of Object.entries(d))
            if (node[k] === undefined)
                node[k] = v;
        if (node.t === "Date" && node.emphasis === undefined)
            node.emphasis = node.form === "numeral" ? "display" : "secondary";
    ...
    const stable = (x) => Array.isArray(x) ? x.map(stable) : x && typeof x === "object" ? Object.fromEntries(Object.keys(x).filter(k => k !== "id").sort().map(k => [k, stable(x[k])])) : x;
    return { tree: out, hash: fnv(JSON.stringify(stable(out))) };
}
// ============================================================ 6. skeleton + signature
function skeletonTokens(n, mode) {
    const a = n;
    const kids = (arr) => arr.flatMap(c => skeletonTokens(c, mode));
    switch (n.t) {
        case "Split": {
        ...
        case "Rail": {
            if (mode === "mobile") {
                if (a.mobile === "hide")
                    return skeletonTokens(a.child, mode);
                const ch = a.mobile === "top" ? [a.rail, a.child] : [a.child, a.rail];
                return ["Stack(", ...kids(ch), ")"];
            }
            return [`Rail:${a.side}:${a.width}(`, ...skeletonTokens(a.rail, mode), "|", ...skeletonTokens(a.child, mode), ")"];
        }
        case "Grid": {
            if (mode === "mobile" && a.mobile === 1)
                return ["Stack(", ...kids(a.children.map((c) => c.child)), ")"];
            return [`Grid:${mode === "mobile" ? a.mobile : a.columns}${a.ruled ? ":ruled" : ""}(`, ...a.children.flatMap((c) => [`Cell:${c.span || 1}${(c.rowSpan || 1) > 1 ? ":tall" : ""}(`, ...skeletonTokens(c.child, mode), ")"]), ")"];
        }
        case "Overlay": {
            if (mode === "mobile" && a.mobile === "stack")
                return ["Stack(", ...skeletonTokens(a.content, mode), "MotifBand", ")"];
            return [`Overlay:${a.anchor}:${a.extent}(`, ...skeletonTokens(a.content, mode), "|", ...skeletonTokens(a.decoration, mode), ")"];
        }
...
function levenshtein(a, b) { const m = a.length, n = b.length; const d = Array.from({ length: m + 1 }, (_, i) => [i, ...Array(n).fill(0)]); for (let j = 1; j <= n; j++)
    d[0][j] = j; for (let i = 1; i <= m; i++)
    for (let j = 1; j <= n; j++)
        d[i][j] = Math.min(d[i - 1][j] + 1, d[i][j - 1] + 1, d[i - 1][j - 1] + (a[i - 1] === b[j - 1] ? 0 : 1)); return d[m][n]; }
function seqSim(a, b) { const L = Math.max(a.length, b.length); return L ? 1 - levenshtein(a, b) / L : 1; }
...
function skeleton(tree, mode) {
    const sec = (k) => tree.sections.find(s => s.kind === k);
    const hero = compress(skeletonTokens(sec("hero").root, mode));
    const r = sec("rsvp"), g = sec("registry");
    return { hero, surfaces: tree.sections.map(s => s.surface), rsvp: r ? compress(skeletonTokens(r.root, mode)) : [], registry: g ? compress(skeletonTokens(g.root, mode)) : [], align: sec("hero").align || "start", heroString: hero.join(" ") };
}
...
function similarity(a, b, mode) {
    ...
    const sa = skeleton(a.tree, mode), sb = skeleton(b.tree, mode);
    ...
    if (a.category !== undefined)
        ...
        s += w.category + w.tone; // tree-only comparison assumes the rest matches (conservative)
    return Math.round(s * 100) / 100;
}
function heroSimilarity(a, b, mode = "desktop") { return Math.round(seqSim(skeleton(a, mode).hero, skeleton(b, mode).hero) * 100) / 100; }
...
function resolveLayout(tree, density) {
    const out = {};
    walk(tree, ({ node }) => {
        const a = node;
        const id = node.id;
        switch (node.t) {
            case "Split":
                ...
                break;
            case "Rail":
                out[id] = { side: a.side, widthPx: SCALES.railPx[a.width], mobile: a.mobile };
                break;
            case "Grid":
                out[id] = { columns: a.columns, mobileColumns: a.mobile, ruled: !!a.ruled, gapPx: SCALES.gapPx[density][a.gap] };
                break;
            case "Cell":
                out[id] = { span: a.span, rowSpan: a.rowSpan };
                break;
            case "Frame":
                out[id] = { rule: a.rule, insetPx: SCALES.insetPx[a.inset], motif: a.motif || null };
                break;
            case "Surface":
                out[id] = { role: a.role, insetPx: SCALES.insetPx[a.inset] };
                break;
            case "Overlay":
                out[id] = { anchor: a.anchor, extentPct: SCALES.extentPct[a.extent], mobile: a.mobile };
                break;
            case "Stack":
                out[id] = { gapPx: SCALES.gapPx[density][a.gap], align: a.align || null };
                break;
            case "Cluster":
                out[id] = { gapPx: SCALES.gapPx[density][a.gap], justify: a.justify };
                break;
            case "MotifField":
                out[id] = { motif: a.motif, extentPct: SCALES.extentPct[a.extent] };
                break;
            case "MotifBand":
                out[id] = { heightPx: SCALES.bandPx[a.height], motif: a.motif || null, fill: a.fill };
                break;
...
function specText(caps) {
    const lines = [];
    const fmt = (p) => p.enum ? p.enum.map(x => JSON.stringify(x)).join("|") : p.type === "motif" ? "{id: MotifId, role: MotifRole}" : p.type === "node" ? "Node" : "?";
    for (const [t, s] of Object.entries(NODE_SPEC)) {
        if (!caps.rsvp && t === "RSVP")
            continue;
        if (!caps.registry && (t === "Registry" || t === "RegistryItem"))
            continue;
        if (!caps.cashFund && t === "CashFund")
            continue;
        if ((t === "Hosts" && !caps.hosts) || (t === "Description" && !caps.description) || (t === "Time" && !caps.time) || (t === "Location" && !caps.location) || (t === "Deadline" && !caps.deadline))
            continue;
...
function rulesText(caps) {
    const req = ["EventTitle exactly once", "Venue exactly once", "Date at least once (at most twice, different forms)"];
    if (caps.rsvp)
        req.push("one rsvp section containing RSVP exactly once");
    if (caps.registry)
        req.push("one registry section containing Registry exactly once, with each RegistryItem kind at most once");
    return [
