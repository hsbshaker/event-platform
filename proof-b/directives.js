...
(function (root) {
  const DIMENSIONS = {
    ...
    weight:     ["start", "end", "symmetric"],                        // where the visual mass sits
    ...
    registry:   ["featured", "grid", "list", "split"],                // registry arrangement
  };
  const PHRASE = {
    ...
    weight: { start: "the visual mass sits at the start (left) edge", end: "the visual mass sits at the end (right) edge", symmetric: "the composition is symmetric and centered" },
    ...
    registry: { featured: "the registry features the gift with the others beside it", grid: "the registry is an even grid", list: "the registry is a ruled list", split: "the registry is a two-column split" },
  };
  ...
  function compliance(tree, d) {
    ...
    const splits = find(h, n => n.t === "Split"), rails = find(h, n => n.t === "Rail"), overlays = find(h, n => n.t === "Overlay");
    const massStart = splits.some(s => Number(s.ratio) >= 62 && find(s.children[0], x => x.t === "EventTitle").length) || rails.some(r => r.side === "start" && r.width !== "thin") || overlays.some(o => o.anchor.endsWith("start"));
    const massEnd = splits.some(s => Number(s.ratio) <= 38 && find(s.children[1], x => x.t === "EventTitle").length) || rails.some(r => r.side === "end") || overlays.some(o => o.anchor.endsWith("end"));
    const centered = (hero.align === "center") && !splits.length && !rails.length;
    c.weight = d.weight === "start" ? massStart : d.weight === "end" ? massEnd : centered;
    ...
    if (reg) { const lay = reg.layout; const featured = find(reg, n => n.t === "RegistryItem" && n.emphasis === "featured").length > 0; c.registry = d.registry === "featured" ? featured : d.registry === "grid" ? lay.t === "Grid" && !featured : d.registry === "list" ? lay.t === "Stack" : lay.t === "Split" && !featured; } else c.registry = null;
    return c;
  }
