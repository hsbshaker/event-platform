...
interface Repair { rule: string; path: string; before?: string; after?: string; kind: "structural" | "coverage" | "capability" | "responsive" | "fit-estimate" | "fit-verified" | "planner" }
...
function validateStructure(tree: CompositionTree, caps: Capabilities): Violation[] {
  ...
  walk(tree, ({ node, path, parent, parentKey, ancestors, sectionIndex }) => {
    ...
    if (node.t === "Frame" || node.t === "Surface") { const boxes = ancestors.filter(a => a.t === "Frame" || a.t === "Surface").length; if (boxes >= 2) add("boxes.depth", path, `${node.t} nested inside ${boxes} boxes`); }
    // motif kind must match its slot: patterns fill fields, bands and frames; arrangements are glyphs and dividers
    if ((node.t === "MotifField" || node.t === "MotifBand" || node.t === "Frame") && (node as any).motif && !PATTERN_MOTIFS.includes((node as any).motif.id)) add("motif.kind", path, `${(node as any).motif.id} is an arrangement, not a pattern`);
    if (node.t === "Glyph" && !ARRANGEMENT_MOTIFS.includes((node as Glyph).motif)) add("motif.kind", path, `${(node as Glyph).motif} is a pattern, not an arrangement`);
    if (node.t === "Rule" && (node as RuleNode).glyphs && !ARRANGEMENT_MOTIFS.includes((node as RuleNode).glyphs!)) add("motif.kind", path, `${(node as RuleNode).glyphs} is a pattern, not an arrangement`);
    // capabilities
    ...
    for (let iter = 0; iter < 40; iter++) {
      ...
      for (const vio of vs) {
        ...
        try {
          ...
          else if (r === "boxes.depth") { const n = getAt(p); const rep: any = n.t === "Frame" ? { t: "Stack", children: [n.child] } : n.child; setAt(p, rep); log(r, p, "structural", n.t, rep.t === "Stack" ? "Stack (box removed)" : "unwrapped"); changed = true; }
          else if (r === "motif.kind") { const n = getAt(p); if (n.t === "Glyph") { log(r, p, "structural", n.motif, "celestial"); n.motif = "celestial"; } else if (n.t === "Rule") { log(r, p, "structural", n.glyphs, "celestial"); n.glyphs = "celestial"; } else { log(r, p, "structural", n.motif.id, "linen"); n.motif = { id: "linen", role: n.motif.role }; } changed = true; }
...
const ATTRACTIVE_TOKENS: { id: string; doc: string; detect: (tree: CompositionTree) => boolean; neutralize: (tree: CompositionTree) => Repair[] }[] = [
  { id: "staggerTitle", doc: "EventTitle.layout stagger or cascade", detect: t => { let f = false; walk(t, ({ node }) => { if (node.t === "EventTitle" && (node as any).layout && (node as any).layout !== "block") f = true; }); return f; },
    neutralize: t => { const out: Repair[] = []; walk(t, ({ node, path }) => { if (node.t === "EventTitle" && (node as any).layout && (node as any).layout !== "block") { out.push({ rule: "planner.attractiveToken", path, kind: "planner", before: (node as any).layout, after: "block" }); (node as any).layout = "block"; } }); return out; } },
  { id: "heroNumeral", doc: "a Date numeral in the hero", detect: t => { let f = false; walk({ version: t.version, sections: [t.sections[0]] }, ({ node }) => { if (node.t === "Date" && (node as DateNode).form === "numeral") f = true; }); return f; },
    neutralize: t => { const out: Repair[] = []; const seen = new Set<string>(); walk({ version: t.version, sections: [t.sections[0]] }, ({ node }) => { if (node.t === "Date") seen.add((node as DateNode).form); });
      walk({ version: t.version, sections: [t.sections[0]] }, ({ node, path }) => { if (node.t === "Date" && (node as DateNode).form === "numeral") { const next = ["month-year", "weekday", "full"].find(f => !seen.has(f)); if (!next) { out.push({ rule: "planner.attractiveToken", path, kind: "planner", before: "numeral", after: "dropped" }); (node as any).t = "Rule"; (node as any).weight = "hairline"; delete (node as any).form; delete (node as any).emphasis; return; } seen.add(next); out.push({ rule: "planner.attractiveToken", path, kind: "planner", before: "numeral", after: next }); (node as any).form = next; if ((node as any).emphasis === "display") (node as any).emphasis = "primary"; } }); return out; } },
  { id: "watermark", doc: "a watermark Monogram or a decorative Date behind the title", detect: t => { let f = false; walk(t, ({ node, parentKey }) => { if (parentKey === "decoration" && (node.t === "Monogram" || node.t === "Date")) f = true; }); return f; },
    neutralize: t => { const out: Repair[] = []; walk(t, ({ node, path, parentKey, parent }) => { if (parentKey === "decoration" && (node.t === "Monogram" || node.t === "Date")) { out.push({ rule: "planner.attractiveToken", path, kind: "planner", before: node.t, after: "MotifField linen" }); (parent as any).decoration = { t: "MotifField", motif: { id: "linen", role: "field" } }; } }); return out; } },
];
